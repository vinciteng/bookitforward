...
function optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		// Select Custom Post Type Box
		case 'posttype_select':
                $post_type  = ($value['post_type'] == '')? 'post':$value['post_type'];
                $posts = new WP_Query(array('post_type' => $post_type, 'posts_per_page' => -1));
                if($posts->have_posts()){
                        $output .= '<select class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" id="' . esc_attr( $value['id'] ) . '">';
                        while($posts->have_posts()){ $posts->the_post();
                                $output .= '<option value="' . get_the_ID() . '" ';
                                $output .= (get_the_ID() == $val)? ' selected':'';
                                $output .= '>' . get_the_title() . '</option>';
                        }
                        wp_reset_postdata();
                        $output .= '</select>';
                } else {
                        $output .= sprintf( __('<p class="description">Please insert an item %s first.</p>'), $post_type);
                }
			break;

		// Select Custom Post Type Box
		case 'taxonomy_select':
                $taxonomy  = ($value['taxonomy'] == '')? 'category':$value['taxonomy'];
                $args = array( 'hide_empty'    => false );
                $tax = get_terms( $taxonomy, $args );
                if($tax){
                        $output .= '<select class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" id="' . esc_attr( $value['id'] ) . '">';
                        foreach( $tax as $item ){ 
                                $output .= '<option value="' . $item->slug . '" ';
                                $output .= ($item->slug == $val)? ' selected':'';
