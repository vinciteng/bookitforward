	...
	class BFL_Backup {
		...
		public function __construct () {
			if ( current_user_can( 'edit_theme_options' ) ) {
				add_action( 'admin_menu', array(&$this, 'add_page'), 23 );
			}
		}

		function add_page() {
			$backup_page = add_theme_page( __( 'Backup Options', 'raakbookoo' ), __( 'Backup Options', 'raakbookoo' ), 'edit_theme_options', 'bfl_backup', array(&$this, 'render_page') );
		}
		...
		public function option_header() {
			...
			$output = '<h2>' . __( 'Theme Options Backup', 'raakbookoo' ) . '</h2>';
			$output .= '<div class="updated settings-error below-h2"><p>' . __( 'Note: This is only to backup your Theme Options!', 'raakbookoo' ) . '</p></div>';
			return $output;
		}
