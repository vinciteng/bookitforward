<?php
/**
 * Plugin Name: Crunchpress Popular Post
...
class Popular_Post extends WP_Widget {
	...
	function  Popular_Post() {
		...
		/* Create the widget. */
		$this->WP_Widget( 'cp_popularpost-widget', __(THEME_NAME_F. ' - Popular Posts', 'crunchpress'), $widget_ops, $control_ops );
	}
					...
					<div class="recent-post-widget-context">
						...
						<div class="recent-post-widget-comment-num post-info-color">
							<?php 
								...
								if( $comments_num > 1 ){ 
									$output = str_replace( '%', number_format_i18n($comments_num), __('% Comments','crunchpress') ); 
									echo $output; 
								}else if( $comments_num == 1 ){ 
									_e('1 Comment', 'crunchpress'); 
								}else{ 
									_e('No Responses.','crunchpress'); 
								}	
							?>							
						</div>
					</div>
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Popular Posts Widget', 'crunchpress'), 'post_cat' => __('0', 'crunchpress'), 'show_num' => '3');
