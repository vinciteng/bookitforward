...
jQuery(function() {
				var Page = (function() {
					var config = {
						...
						jQuerynavPrev : jQuery( '#bb-nav-prev' ),
						jQueryfolder : jQuery( '#folder' ),
						jQueryfolderOpen : jQuery( '#folder > div.folder-cover > span' ),
						jQueryfolderClose : jQuery( '#folder > div.folder-inner > a' ),
						transEndEventNames : {
						...
						initEvents = function() {
							config.jQuerynavNext.on( 'click', function() {
								checkFolder( function() {
									config.bb.next();	
								} );
								return false;
							} );
							config.jQuerynavPrev.on( 'click', function() {
								checkFolder( function() {
									config.bb.prev();
								} );
								return false;
							...
							config.jQuerybookBlock.children().on( {
							...
							// folder
							config.jQueryfolderOpen.on( 'click', function() {
								var jQueryfolder = jQuery( this ).closest( 'div.folder' ),
										open = jQueryfolder.data( 'isOpen' );
							...
							config.jQueryfolderClose.on( 'click', function() {
								var jQueryfolder = jQuery( this ).closest( 'div.folder' ),
										open = jQueryfolder.data( 'isOpen' );
								if( config.bb.isActive() ) return false;
								if( open ) {
						...
						openFolder	= function() {
							var jQueryfold = config.jQueryfolder.find( 'div.folder-fold' ).css( config.transitionProperty ),
								jQueryinner = config.jQueryfolder.find( 'div.folder-inner' ),
								transEndEventName = config.transEndEventNames[ Modernizr.prefixed( 'transition' ) ];
							setTimeout( function() {
								jQueryfold.css( 'transform', 'rotateY(180deg)' ).on( transEndEventName , function() {
									jQueryfold.off( transEndEventName ).css( 'z-index', 1 ).css( 'transition', 'none' ).css( 'transform', 'rotateY(-180deg)' );
									jQueryinner.css( 'transform', 'translateX(310px)' );
								} );
								config.jQueryfolder.data( 'isOpen', true );
							}, 0 );
						},
						closeFolder	= function() {
							var jQueryfold = config.jQueryfolder.find( 'div.folder-fold' ),
									jQueryinner = config.jQueryfolder.find( 'div.folder-inner' ),
									transEndEventName = config.transEndEventNames[ Modernizr.prefixed( 'transition' ) ];
							jQueryinner.css( 'transform', 'translateX(0px)' ).on( transEndEventName , function() {
								jQueryinner.off( transEndEventName );
								jQueryfold.css( 'transform', 'rotateY(180deg)' );
								setTimeout( function() {
									jQueryfold.css( config.transitionProperty ).css( {
										transform	: 'rotateY(0deg)',
										zIndex		: 4
									} ).on( transEndEventName , function() {
										jQueryfold.off( transEndEventName );
									} );
								}, 0 );
							} );
							config.jQueryfolder.data( 'isOpen', false );
						},
						checkFolder	= function( callback ) {
							var jQueryfold = config.jQueryfolder.find( 'div.folder-fold' ),
									transEndEventName = config.transEndEventNames[ Modernizr.prefixed( 'transition' ) ];
							if( config.jQueryfolder.data( 'isOpen' ) ) {
								jQueryfold.on( transEndEventName , function() {
									jQueryfold.off( transEndEventName );
									callback.call();
								} );
								closeFolder();
