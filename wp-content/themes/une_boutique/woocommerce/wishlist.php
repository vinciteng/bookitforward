...
<form id="yith-wcwl-form" action="<?php echo esc_url( $yith_wcwl->get_wishlist_url() ) ?>" method="post">
    ...
    if( !empty( $wishlist_title ) )
        { echo apply_filters( 'yith_wcwl_wishlist_title', '<h3 class="no-margin-top"><small>' . $wishlist_title . '</small></h3>' ); }
    ...
    <table class="shop_table cart wishlist_table" cellspacing="0">
    	<thead>
    		<tr>
    			...
    			<th data-hide="phone,tablet" class="product-thumbnail"></th>
    			<th class="product-name"><span class="nobr"><?php _e( 'Product Name', 'yit' ) ?></span></th>
                <?php if( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : ?><th data-hide="phone" class="product-price"><span class="nobr"><?php _e( 'Unit Price', 'yit' ) ?></span></th><?php endif ?>
    			<?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?><th data-hide="phone,tablet"><span class="nobr"><?php _e( 'Stock Status', 'yit' ) ?></span></th><?php endif ?>
            ...
            if( count( $wishlist ) > 0 ) :
                foreach( $wishlist as $values ) :   
                    ...
                    <tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
                        <td class="product-remove"><div><a href="javascript:void(0)" onclick="remove_item_from_wishlist( '<?php echo esc_url( $yith_wcwl->get_remove_url( $values['ID'] ) )?>', 'yith-wcwl-row-<?php echo $values['ID'] ?>');" class="remove" title="<?php _e( 'Remove this product', 'yit' ) ?>"><span class="ub-icon-trashcan"></span></a></td>
