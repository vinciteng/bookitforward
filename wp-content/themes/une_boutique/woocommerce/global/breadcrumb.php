...
<nav class="shop-items-nav alignright">
	<ul class="unstyled inline-list">
		<?php if ( is_product() ) { ?>
		<li class="next">
			<?php previous_post_link('%link', '<i class="ub-icon-arrow-right7"></i>', false, '') ;?>
		</li>
		<li class="previous">
			<?php previous_post_link('%link', '<i class="ub-icon-arrow-left7"></i>', false, '') ;?>
		</li>
		<?php } ?>
		<?php if ( is_shop() ) { ?>
...
if ( strpos($url,'min_price') !== false || strpos($url,'max_price') !== false || strpos($url,'filter_') !== false ) {
	$has_filters = "";
} else {
